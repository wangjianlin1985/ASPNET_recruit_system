...
namespace DAL
{
    ...
    public class dalUserInfo
    {
        ...
        {
            string sql = "insert into UserInfo(user_name,password,name,gender,birthDate,userPhoto,xl,telephone,email,address,jianliFile,smrzzl,shzt,regTime) values(@user_name,@password,@name,@gender,@birthDate,@userPhoto,@xl,@telephone,@email,@address,@jianliFile,@smrzzl,@shzt,@regTime)";
            /*构建sql参数*/
            SqlParameter[] parm = new SqlParameter[] {
             ...
             new SqlParameter("@xl",SqlDbType.VarChar),
             new SqlParameter("@telephone",SqlDbType.VarChar),
             new SqlParameter("@email",SqlDbType.VarChar),
             new SqlParameter("@address",SqlDbType.VarChar),
             new SqlParameter("@jianliFile",SqlDbType.VarChar),
             new SqlParameter("@smrzzl",SqlDbType.VarChar),
            ...
            parm[5].Value = userInfo.userPhoto; //学生照片
            parm[6].Value = userInfo.xl; //学历
            parm[7].Value = userInfo.telephone; //联系电话
            parm[8].Value = userInfo.email; //邮箱
            parm[9].Value = userInfo.address; //家庭地址
            parm[10].Value = userInfo.jianliFile; //简历文件
            parm[11].Value = userInfo.smrzzl; //实名认证资料
            parm[12].Value = userInfo.shzt; //审核状态
            parm[13].Value = userInfo.regTime; //注册时间
        ...
        public static ENTITY.UserInfo getSomeUserInfo(string user_name)
        {
            ...
            if (DataRead.Read())
            {
                ...
                userInfo.xl = DataRead["xl"].ToString();
                ...
                userInfo.jianliFile = DataRead["jianliFile"].ToString();
                userInfo.smrzzl = DataRead["smrzzl"].ToString();
                userInfo.shzt = DataRead["shzt"].ToString();
        ...
        {
            string sql = "update UserInfo set password=@password,name=@name,gender=@gender,birthDate=@birthDate,userPhoto=@userPhoto,xl=@xl,telephone=@telephone,email=@email,address=@address,jianliFile=@jianliFile,smrzzl=@smrzzl,shzt=@shzt,regTime=@regTime where user_name=@user_name";
            /*构建sql参数信息*/
            SqlParameter[] parm = new SqlParameter[] {
             new SqlParameter("@password",SqlDbType.VarChar),
             new SqlParameter("@name",SqlDbType.VarChar),
            ...
            parm[5].Value = userInfo.xl;
            parm[6].Value = userInfo.telephone;
            parm[7].Value = userInfo.email;
            parm[8].Value = userInfo.address;
            parm[9].Value = userInfo.jianliFile;
            parm[10].Value = userInfo.smrzzl;
            parm[11].Value = userInfo.shzt;
            parm[12].Value = userInfo.regTime;
            parm[13].Value = userInfo.user_name;
            /*执行更新*/
            return (DBHelp.ExecuteNonQuery(sql, parm) > 0) ? true : false;
        }
