...
public partial class UserInfo_UserInfoController : System.Web.UI.Page
{
    ...
    protected void addUserInfo()
    {
        ...
        userInfo.xl = Request["userInfo.xl"];
        ...
        userInfo.jianliFile = handleFileUpload("jianliFileFile");
        userInfo.smrzzl = handleFileUpload("smrzzlFile");
        userInfo.shzt = Request["userInfo.shzt"];
    ...
    protected void updateUserInfo()
    {
        ...
        userInfo.email = Request["userInfo.email"];
        userInfo.address = Request["userInfo.address"];
        userInfo.jianliFile = Request["userInfo.jianliFile"];
        string jianliFilePath = handleFileUpload("jianliFileFile");
        if (jianliFilePath != "") userInfo.jianliFile = jianliFilePath;
        userInfo.smrzzl = Request["userInfo.smrzzl"];
        string smrzzlPath = handleFileUpload("smrzzlFile");
        if (smrzzlPath != "") userInfo.smrzzl = smrzzlPath;
        userInfo.shzt = Request["userInfo.shzt"];
    ...
    protected void getUserInfo()
    {
        ...
        jsonUserInfo.Put("xl", userInfo.xl);
        ...
        jsonUserInfo.Put("jianliFile", userInfo.jianliFile);
        jsonUserInfo.Put("smrzzl", userInfo.smrzzl);
        jsonUserInfo.Put("shzt", userInfo.shzt);
    ...
    //处理一般文件上传
    protected string handleFileUpload(string fileKeyName)
    {
        string filePath = "";
        HttpPostedFile upFile = Request.Files[fileKeyName];
        if (upFile.ContentLength > 0)
        {
            //获取文件的扩展名
            string fileExt = Path.GetExtension(upFile.FileName);
            string saveFileName = DAL.Function.MakeFileName(fileExt);
            filePath = "FileUpload/" + saveFileName;/*图片路径*/
